<?php /* Template Name: All Locations */ ?>
					...
					<div class="entry-content">
									...
									<div class="col-6 col-sm-4 <?= ($layout_options['sidebar']) ? 'col-xl-3' : 'col-lg-3 col-xl-2' ?> p-2">
										<a class="text-decoration-none" href="<?php echo get_term_link($parent); ?>">
											<div class="cmd-taxonomy-card card h-100 p-2 rounded-4 shadow-sm">
												...
												<div class="card-footer text-center py-1 px-0"><small><?php echo count(query_posts(array('locations' => $parent->slug, 'posts_per_page' => -1, 'post__not_in' => $exclude_coupons))) . ' ' . __('Offers', "clipmydeals"); ?></small></div>
											</div>
										</a>
									</div>
								<?php } ?>
						...
						<?php
								...
								foreach ($parent_categories as $parent) { // hierarchical foreach starts
									$parent_count = count(query_posts(array('locations' => $parent->slug, 'posts_per_page' => -1, 'post__not_in' => $exclude_coupons)));
						?>
