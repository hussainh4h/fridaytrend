...
$brand_custom_fields = cmd_get_taxonomy_options($term->term_id, 'brands');
...
?>
	<div class="cmd-taxonomy-brands <?= $layout_options['container'] ?> pt-5">
		<div class="row mx-0">
		<?php
	}
		...
		<section id="primary" class="brand-page taxonomy-page content-area order-md-1 <?= ($layout_options['sidebar']) ? 'col-sm-12 col-lg-8 col-lg-9' : 'col' ?>">
			<main id="main" class="site-main" role="main">

				<header class="row page-header mb-2">
					<?php if (!empty($brand_custom_fields['brand_image'])) { ?>
						<div class="col-sm-3 my-1">
							<img class="img-thumbnail img-fluid" src="<?php echo $brand_custom_fields['brand_image']; ?>" />
						</div>
					<?php } ?>
					<div class="col my-1">
						<div class="alert alert-secondary" id="brand-archive-alert">
							...
							$custom_title = $brand_custom_fields['page_title'];
				...
				<?php
				if (get_theme_mod('article_placement', 'bottom') == 'top' and !empty($brand_custom_fields['brand_intro']))
					echo '<div class="card"><article class="card-body">' . do_shortcode(stripslashes($brand_custom_fields['brand_intro'])) . '</article></div>';
				?>
				...
				if (get_theme_mod('article_placement', 'bottom') == 'bottom' and !empty($brand_custom_fields['brand_intro']))
					echo '<div class="card"><article class="card-body">' . do_shortcode(stripslashes($brand_custom_fields['brand_intro'])) . '</article></div>';
				?>
