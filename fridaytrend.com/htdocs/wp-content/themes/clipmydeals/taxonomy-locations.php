...
$location_custom_fields = cmd_get_taxonomy_options($term->term_id, 'locations');
...
?>
	<div class="cmd-taxonomy-locations <?= $layout_options['container'] ?> pt-5">
		<div class="row mx-0">
		<?php
	}
		...
		<section id="primary" class="location-page taxonomy-page content-area order-md-1 <?= ($layout_options['sidebar']) ? 'col-sm-12 col-lg-8 col-lg-9' : 'col' ?>">
			<main id="main" class="site-main" role="main">
				...
				<header class="row page-header mb-2">
					<div class="col my-1">
						<div class="alert alert-secondary" id="location-archive-alert">
							...
							$custom_title = $location_custom_fields['page_title'];
				...
				<?php
				if (get_theme_mod('article_placement', 'bottom') == 'top' and !empty($location_custom_fields['location_intro']))
					echo '<div class="card"><article class="card-body">' . do_shortcode(stripslashes($location_custom_fields['location_intro'])) . '</article></div>';
				?>
				...
				if (get_theme_mod('article_placement', 'bottom') == 'bottom' and !empty($location_custom_fields['location_intro']))
					echo '<div class="card"><article class="card-body">' . do_shortcode(stripslashes($location_custom_fields['location_intro'])) . '</article></div>';
				?>
